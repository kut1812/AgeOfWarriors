...
bool MainMenuScene::init() {
	...
	buttonHighScores->addTouchEventListener(
		[&](Ref* sender, ui::Widget::TouchEventType type) {
			...
			case ui::Widget::TouchEventType::BEGAN:
				break;
			case ui::Widget::TouchEventType::ENDED:
				Director::getInstance()->replaceScene(TransitionFade::create(2, HighScoreScene::createScene()));
				break;
			case ui::Widget::TouchEventType::CANCELED:
				break;
			default:
	...
	labelExit->setPosition(Vec2(visibleSize.width / 2, visibleSize.height * 0.1));
	labelExit->setScale(visibleSize.height / labelStartGame->getContentSize().height * 0.05);
	this->addChild(labelExit, 6);
	buttonExit->setPosition(Vec2(visibleSize.width / 2, visibleSize.height * 0.1));
